...
class LinkedList:
    ...
    def set_head_node(self, node_data):
        if self.get_head_node() == None:
            ...
            raise ValueError(
                "There is already a head node, please use the set_new_head() method")
    ...
    def get_previous_node(self, node):
        ...
        if starting_node == node:
            raise ValueError(
                "The Node you are trying to find is the head of the list (There is no previous Node)")
    ...
    def add_node(self, node_to_add):
        ...
        if starting_node is None:
            raise ValueError(
                "The list you are trying to add to is empty, please add a new head Node to start a list")
    ...
    def set_new_head(self, node_to_set):
        ...
        if head_node is not None:
            ...
            # Set the new Node to point at the previous head Node
            new_node.set_next_node(head_node)
        elif head_node is None:
            # Sets the head Node to the node to set as head if there is not head Node
            self.set_head_node(Node(node_to_set))

    # Removes the Node from the list that contains the value of the argument passed in
        ...
        if self.get_head_node() is not None:
            ...
            while starting_node is not None:  # Starts at the head Node
                if starting_node.get_nodes_value() == node_to_remove:
                    # Getting the next Node to set Previous Node to point to next
                    starting_node_next = starting_node.get_next_node()
    ...
    def stringify_list(self):
        ...
        while starting_node is not None:
            if starting_node.get_next_node() == None:
                string_list += str(starting_node.get_nodes_value())
                starting_node = starting_node.get_next_node()
            else:
                string_list += str(starting_node.get_nodes_value()) + ", "
                starting_node = starting_node.get_next_node()
